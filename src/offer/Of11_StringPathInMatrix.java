package offer;
public class Of11_StringPathInMatrix {
...
//        }
    ...
    private boolean hasPathCore(char[] matrix, int rows, int cols, int row, int col, char[] str, int pathLength,
                                boolean[] isVisited) {
        if (row < 0 || col < 0 || row >= rows || col >= cols    //todo 判断边界
                || isVisited[row * cols + col] == true          //todo 标记已经访问
                || str[pathLength] != matrix[row * cols + col]) //todo 判断字节是否相等
            return false;
        ...
        if (!hasPath) {
            isVisited[row * cols + col] = false;//todo 回溯-上面已经判断在包含此字符下所有的情况都不满足，所以退出包含该字符的情况。
        }
        return hasPath;
    }
    ...
    public static void main(String[] args) {
        Of11_StringPathInMatrix demo = new Of11_StringPathInMatrix();
        demo.test1();
        demo.test2();
        demo.test3();
        demo.test4();
