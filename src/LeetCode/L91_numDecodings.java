...
思路
 ...
 1. 那么如果字符串的第i-1位和第i位能组成一个10到26的数字，说明我们是在第i-2位的解码方法上继续解码。
 2. 如果字符串的第i-1位和第i位不能组成有效二位数字，而且第i位不是0的话，说明我们是在第i-1位的解码方法上继续解码。
...
//            System.out.println("===>"+s.substring(i-1, i));
            if(Integer.parseInt(s.substring(i-1, i)) != 0){//在
                dp[i] += dp[i - 1];
            }
        }
        return dp[s.length()];
