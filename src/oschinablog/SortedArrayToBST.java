 ...
 *
 * 思路:降序储存和顺序储存都会得到退化树。最有效保持树平衡的是，取数组中值（对于无重复元素的有序数组，中值就是数组中间元素）作为树根的值，将数组分为左右两个子数组，左子数组的数值均小于根节点，右子数组的值均大于根节点。将左子数组的中间元素作为根节点的左子树，将右子树的中间元素作为根节点的右子树，由此，形成递归关系，就能得到平衡的二叉搜索树
 *
